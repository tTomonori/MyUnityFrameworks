...
public class TextWriter : MonoBehaviour {
    ...
    private void readTag() {
        ...
        switch (tTag[0]) {
            ...
            case "clear"://文字の書き込みを停止し,再開時に表示リセット
                ...
                break;
            case "image"://画像
                Image tImage = createImage(tTag[1]);
                writeImage(tImage);
                break;
        }
    }
    ...
    //画像を表示
    public void writeImage(Image aImage) {
        if (mLines[0].add(aImage)) return;
        createNewLine();
        mLines[0].add(aImage);
    }
    //新しい行を生成
    ...
    public class Line {
        ...
        //画像を追加
        public bool add(Image aImage) {
            //はみ出る
            if (mRect.sizeDelta.x < mRight + aImage.preferredWidth / TextWriter.kScale) return false;
            //表示
            aImage.transform.SetParent(mRect.transform, false);
            aImage.rectTransform.localPosition = new Vector3(mRight, -mHeight + aImage.preferredHeight / TextWriter.kScale);
            //上側で見切れても映るように

            mRight += aImage.preferredWidth / TextWriter.kScale;
            if (mHeight < aImage.preferredHeight / TextWriter.kScale) {
                mHeight = aImage.preferredHeight / TextWriter.kScale;
                correctHeight();
            }
            return true;
        }
